...
import subprocess
#TODO put this and duplicate from cae into helpers
...
class ReIDModel(nn.Module):
    def __init__(self, features, cae_latent_dim, num_classes, num_species, in_dim, num_hog_bins, 
                 hidden_dim=256, gnn_out_dim=256, emb_dim=512, use_hybrid_pooling=True, 
                 edge_strategy="spatial", k_neighbors=5, 
                 gnn_layers=3, gnn_type="gatv2", use_jk=True, 
                 jk_mode="attention", pooling_type="gem",            
                 drop_node=0.10, drop_edge=0.15, drop_modality=0.0): 
        super().__init__()
        self.save_params = {
            'features': features, 'num_classes': num_classes, 'cae_latent_dim': cae_latent_dim,
            'num_species': num_species, 'in_dim': in_dim, 'hidden_dim': hidden_dim,
            'gnn_out_dim': gnn_out_dim, 'emb_dim': emb_dim, 'use_hybrid_pooling': use_hybrid_pooling,
            'edge_strategy': edge_strategy, 'k_neighbors': k_neighbors, 'num_hog_bins': num_hog_bins,
            'gnn_layers': gnn_layers, 'gnn_type': gnn_type, 'use_jk': use_jk, 
            'jk_mode': jk_mode, 'pooling_type': pooling_type,        
            'drop_node': drop_node, 'drop_edge': drop_edge, 'drop_modality': drop_modality
        }
        
        self.encoder = GNNEncoder(
            num_hog_bins=num_hog_bins, features=features, cae_latent_dim=cae_latent_dim, 
            in_dim=in_dim, hidden_dim=hidden_dim, out_dim=gnn_out_dim, 
            edge_strategy=edge_strategy, k_neighbors=k_neighbors,
            gnn_layers=gnn_layers, gnn_type=gnn_type, use_jk=use_jk, jk_mode=jk_mode,
            pooling_type=pooling_type, drop_node=drop_node,         
            drop_edge=drop_edge, drop_modality=drop_modality
        )
    ...
    def load(checkpoint_path, args, device='cpu'):
        ...
        hyperparams['num_hog_bins'] = hyperparams.get('num_hog_bins', getattr(args, 'num_hog_bins', 9))
        
        # NEW FAILSAFES FOR EVALUATION
        hyperparams['jk_mode'] = hyperparams.get('jk_mode', 'attention')
        hyperparams['pooling_type'] = hyperparams.get('pooling_type', 'gem')
        hyperparams['drop_node'] = hyperparams.get('drop_node', 0.10)
        hyperparams['drop_edge'] = hyperparams.get('drop_edge', 0.15)
        hyperparams['drop_modality'] = hyperparams.get('drop_modality', 0.0)
...
def train_one_epoch(loader, model, arcface_loss, scaler, optimizer, margin=1.0):
    ...
    for data in loader:
        ...
        with torch.amp.autocast(device_type="cuda"):
            ...
            # If args.ortho_weight is 0.0, this gracefully turns the loss off for the ablation!
            loss_ortho = strict_orthogonality_loss(id_emb, species_emb) * args.ortho_weight
...
def main(args):
    # Setup Paths
    print(f"Locking RNG Seed to {args.seed} for reproducibility...")
    ...
    if getattr(args, 'use_cutouts', False):
        print(f"\n[ CUTOUT PHASE ] Ensuring training cutouts exist in {args.cutout_root}...")
        generate_offline_cutouts(args, train_df)
    ...
    train_loader = DataLoader(
        train_dataset, 
        batch_size= int(args.batch_size), # e.g., 128 or 256
        shuffle=True, 
    ...
    model = ReIDModel(
        num_classes=num_train_classes, num_species=num_species, in_dim=dynamic_in_dim, 
        hidden_dim=512, gnn_out_dim=256, 
        emb_dim=args.emb_dim,                  
        pooling_type=args.pooling_type,        
        use_hybrid_pooling=False, 
        edge_strategy=args.edge_strategy, k_neighbors=args.k_neighbors, 
        features=args.features, cae_latent_dim=args.cae_latent_dim, num_hog_bins=args.num_hog_bins,
        gnn_layers=args.gnn_layers, gnn_type=args.gnn_type, 
        use_jk=args.use_jk, jk_mode=args.jk_mode 
    ).to(device)
...
if __name__ == "__main__":
    ...
    # --- ABLATION FLAGS ---
    parser.add_argument("--gnn_layers", type=int, default=3, help="Number of message passing layers (1, 2, or 3)")
    parser.add_argument("--gnn_type", type=str, default="gatv2", choices=["gatv2", "gcn"], help="Type of GNN layer")
    parser.add_argument("--use_jk", action="store_true", help="Enable Dynamic Jumping Knowledge (requires >1 layer)")
    parser.add_argument("--jk_mode", type=str, default="attention", choices=["attention", "mean"], help="How to compress JK")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")
    parser.add_argument("--pooling_type", type=str, default="gem", choices=["gem", "mean", "max"], help="Graph pooling method")
    parser.add_argument("--emb_dim", type=int, default=512, help="Dimension of the final global embedding")
    parser.add_argument("--ortho_weight", type=float, default=1.0, help="Weight of the strict orthogonality loss")
    parser.add_argument("--drop_node", type=float, default=0.10, help="Node dropout probability")
    parser.add_argument("--drop_edge", type=float, default=0.15, help="Edge dropout probability")
    parser.add_argument("--drop_modality", type=float, default=0.0, help="Modality feature dropout probability")
    parser.add_argument("--batch_size", type=float, default=512, help="Modality feature dropout probability")
    parser.add_argument("--use_cutouts", action="store_true", help="Use U2-Net segmented backgrounds")
    args = parser.parse_args()


    if args.use_cutouts:
        args.img_root = args.root_dir               # The muddy originals
        args.cutout_root = args.root_dir + "-cutouts" # The black-background targets
        args.root_dir = args.cutout_root            # Redirect the PyG dataset here!
        print(f"--> [DATA SHIFT] Using Cutout Dataset: {args.root_dir}")
    ...
    try:
        ...
        import traceback
        import datetime
        crash_log = os.path.join(args.checkpoint_dir, "crash_report.txt")
        with open(crash_log, "a") as f:
            f.write(f"\n[{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Y')}] CHILD SCRIPT EXCEPTION (Training):\n")
            f.write(traceback.format_exc() + "\n")
            f.write("-" * 50 + "\n")
        raise e # Throw it back to the orchestrator so it knows to stop
